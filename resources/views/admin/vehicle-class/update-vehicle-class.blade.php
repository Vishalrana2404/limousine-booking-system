@extends('components.layout')
@section('content')
<form id="updateVehicleClassForm" method="post" action="{{ route('update-vehicle-class',$vehicleClass) }}">
    @csrf
    <div class="content-wrapper">
        <section class="content-header border-bottom">
            <div class="container-fluid">
                <input type="hidden" id="vehicleClassId" value="{{$vehicleClass->id}}">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <a class="dark-color back-btn" href="{{ route('vehicle-class') }}" title="Vehicle Class">
                            <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <rect x="25" y="25" width="25" height="25" transform="rotate(-180 25 25)" fill="url(#pattern0_31_250)" />
                                <defs>
                                    <pattern id="pattern0_31_250" patternContentUnits="objectBoundingBox" width="1" height="1">
                                        <use xlink:href="#image0_31_250" transform="scale(0.0078125)" />
                                    </pattern>
                                    <image id="image0_31_250" width="128" height="128" xlink:href="data:image/png;base64," />
                                </defs>
                            </svg>
                            Vehicle Class
                        </a>
                        <h6 class="head-sm medium">Edit Vehicle Class</h6>
                    </div>
                    <div class="col-md-4">
                        <button type="submit" id="updateVehicleClassFormButton" class="btn btn-outline-primary float-right mx-2" title="Save">Save</button>
                    </div>
                </div>
            </div>
        </section>
        <section class="content">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12 ps-0">
                        <div class="card no-box-shadow">
                            <div class="card-header px-0">
                                <div class="row">
                                    <h3 class="head-sm medium">Basic Information</h3>
                                </div>
                            </div>
                            <div class="card-body px-0">
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="name">Class Name <span class="text-danger">*</span></label>
                                            <input type="text" id="name" name="name" value="{{ $vehicleClass->name}}" class="form-control @error('name') is-invalid @enderror" placeholder="Vehicle Name" autofocus>
                                            @error('name')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                            @enderror
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="seating_capacity">Seating Capacity <span class="text-danger">*</span></label>
                                            <input type="text" id="seating_capacity" name="seating_capacity" value="{{ $vehicleClass->seating_capacity }}" class="form-control @error('seating_capacity') is-invalid @enderror" placeholder="Seating Capacity" autofocus>
                                            @error('seating_capacity')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                            @enderror
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="total_luggage">Total Luggage <span class="text-danger">*</span></label>
                                            <input type="text" id="total_luggage" name="total_luggage" value="{{ $vehicleClass->total_luggage }}" class="form-control @error('total_luggage') is-invalid @enderror" placeholder="Total Luggage" autofocus>
                                            @error('total_luggage')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                            @enderror
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="total_pax">Total Pax <span class="text-danger">*</span></label>
                                            <input type="text" id="total_pax" name="total_pax" value="{{$vehicleClass->total_pax}}" class="form-control @error('total_pax') is-invalid @enderror" placeholder="Total Pax" autofocus>
                                            @error('total_pax')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                            @enderror
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="status">Status</label>
                                            <select name="status" id="status" class="form-control form-select custom-select @error('status') is-invalid @enderror">
                                                <option value="ACTIVE" {{ $vehicleClass->status === "ACTIVE" ? 'selected' : '' }}>Active</option>
                                                <option value="INACTIVE" {{ $vehicleClass->status === "INACTIVE" ? 'selected' : '' }}>Inactive</option>

                                            </select>
                                            @error('status')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                            @enderror
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    {{-- <div class="col-md-6">
                    <div class="card no-box-shadow border">
                        <div class="card-header">
                            <h3 class="head-sm medium">Blackout Period</h3>
                        </div>
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table m-0">
                                    <thead>
                                        <tr>
                                            <th style="width: 10px;">#</th>
                                            <th>From Date</th>
                                            <th>To Date</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td>1.</td>
                                            <td>20th May 2024, 2pm</td>
                                            <td>25th May 2024, 11.59pm</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div> --}}
                </div>
        </section>
    </div>
</form>
@vite(['resources/js/VehicleClass.js'])
<script>
    const props = {};
</script>
@endsection