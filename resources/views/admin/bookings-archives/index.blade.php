...
@section('content')
                ...
                <div class="row align-items-center g-3">
                    @if ($userTypeSlug === null || in_array($userTypeSlug, ['admin', 'admin-staff']))
                        ...
                        @else
                            <div class="col-xl-4">
                    @endif
                    <h1 class="semibold head-sm">Archived Bookings</h1>
                    <p class="normal text-xs">Archived Booking Management</p>
                </div>
                <div class="col-xl-2">
        ...
        <div class="card m-0">
            ...
            <div class="card-body p-0">
                <div class="table-responsive custom-table table-scroll-height">
                    <table id="bookingArchiveTable" class="table table-head-fixed text-nowrap table-hover m-0 large-table">
                        <thead>
                            <tr>
                                @if ($userTypeSlug === null || in_array($userTypeSlug, ['admin', 'admin-staff']))
                                    @if($user->department == 'Management')
                        ...
                        <tbody>
                            @include('admin.bookings-archives.partials.bookings-archives-listing')
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    @vite(['resources/js/BookingsArchive.js'])
    <script>
        const props = {
            routes: {
                filterBookingsArchive: "{{ route('filter-bookings-archives') }}",
                permanentDeleteBookings: "{{ route('permanent-delete-bookings') }}"
            },
